...
#def __init__(self, {% for property in range(entity.properties|length) %} {{properties[property].name}} {% if properties[property+1]%}, {% endif %} {% endfor %}):
...
def main(debug=False):
    ...
    person_model = entity_mm.model_from_file(join(this_folder, 'raqueta.ent'))
    ...
    template = jinja_env.get_template('prueba.template')

    for entity in person_model.entities:
        # For each entity generate java file
        with open(join(srcgen_folder,
                       "prueba%s.py" % entity.name.capitalize()), 'w') as f:
            f.write(template.render(entity=entity))
